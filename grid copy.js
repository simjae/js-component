...
function libraryLoad() {
  ...
  let imageArr = new Array(); // var img = document.createElement('img');        
  for (var i in libraryImg.data) {
  ...
  library.forEach((item) => {
    item.addEventListener('dragstart', (ev) => {
      if (ev.dataTransfer.effectAllowed === 'uninitialized') {
        ...
        //$('.product-wrap').css('background-image', 'url(' + ev.target.src + ')');
      }
    });
  });
}
function libraryAddEvent() {
  let gridCard = document.querySelectorAll('.grid');
...
function libraryDrop(ev) {
  ...
  if (ev.dataTransfer.effectAllowed === 'copyMove') {
  ...
  [...productWrap.children].forEach((el) =>{
    ...
    if(childCnt > 0){
      category = el.children[0].dataset.category;
        ...
        data.push({"idx":el.dataset.idx, "category":category, "position":el.dataset.position})
      }
  });
  let productcode = productTemp.productcode;
  productTemp.data = data;
  document.querySelector(`#${productcode}`).parentElement.parentElement.parentElement.dataset.productcode = productcode;
  console.log('productTemp',productTemp);
  var jsonDataString = JSON.stringify(productTemp) ;
  document.querySelector(`#${productcode}`).parentElement.parentElement.previousSibling.value =jsonDataString;
...
$(document).ready(function () {
  ...
  function generateElements(amount) {
    ...
    for (let i = 0, len = amount || 1; i < amount; i++) {
      ...
      let itemTemplate =
        "" +
        '<div draggable="true" class="item h' +
        height +
        " w" +
        width +
        " " +
        ...
        '<div class="card" onclick="gridLibraryClick(e)">' +
        '<div class="card-id">' +
        id +
        "</div>" +
